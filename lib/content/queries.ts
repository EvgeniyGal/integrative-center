...
export async function getVisiblePolicies() {
  ...
  return db
    .select()
    .from(policies)
    .where(eq(policies.visible, true))
    .orderBy(asc(policies.sortOrder), asc(policies.createdAt));
}
...
export async function getAboutPolicies() {
  ...
  return db
    .select()
    .from(policies)
    .where(and(eq(policies.visible, true), eq(policies.showOnAbout, true)))
    .orderBy(asc(policies.sortOrder), asc(policies.createdAt));
}
...
export async function getPolicyBySlug(slug: string) {
  ...
  const rows = await db
    .select()
    .from(policies)
    .where(and(eq(policies.slug, slug), eq(policies.visible, true)))
    .limit(1);
  return rows[0] ?? null;
}
...
export async function getDashboardCounts() {
  const [q, s, t, a, brands, products, categories, stores, p] =
    await Promise.all([
      ...
      db.select({ value: count() }).from(policies),
    ]);
